...
public class VivoxVoiceManager : MonoBehaviour
{
	...
	private VivoxUnity.Client _client => VivoxService.Instance.Client;
	...
	public async UniTask LoginAsync(string displayName = null)
	{
		loggedIn = false;
		OnUserLoggedInEvent += () => loggedIn = true;
		Login(displayName);
	...
	public void DisconnectAllChannels()
	{
		...
		{
			foreach (IChannelSession channelSession in ActiveChannels)
			{
	...
	public void SetOthersMute(bool shouldMute)
	{
		// チャンネルに入ってすぐだと0が評価されてしまう
		// Debug.Log(ActiveChannels.Single().Participants.Count);

		/*/ 現在はコントローラをミュートすることで対処している
		LoginSession.GetChannelSession(ActiveChannels.Single().Channel)
			.Participants.ForEach(p => p.LocalMute = shouldMute);
		//*/
	}
	...
	private void OnParticipantAdded(object sender, KeyEventArg<string> keyEventArg)
	{
		...
		string username = participant.Account.Name;
	...
	private void OnParticipantRemoved(object sender, KeyEventArg<string> keyEventArg)
	{
		...
		if (participant.IsSelf)
		{
			...
			// Remove session.
			ILoginSession user = _client.GetLoginSession(m_Account);
